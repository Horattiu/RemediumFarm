...
async function createWorkplaceAndAdmin() {
  try {
    console.log("═══════════════════════════════════════");
    console.log("🔄 CREARE WORKPLACE DEPOZIT ȘI ADMIN");
    console.log("═══════════════════════════════════════\n");

    // 1️⃣ Creează workplace "remedium depozit"
    console.log("📝 Creare workplace 'remedium depozit'...");
    
    // Verifică dacă workplace-ul există deja
    let workplace = await Workplace.findOne({ 
      $or: [
        { name: "remedium depozit" },
        { name: { $regex: /remedium.*depozit/i } }
      ]
    });
    ...
    if (workplace) {
      ...
      workplace = new Workplace({
        name: "remedium depozit",
        code: "DEP-001", // Cod unic pentru workplace
    ...
    // 2️⃣ Creează admin user "admindepozit"
    console.log("\n📝 Creare admin user 'admindepozit'...");
    
    // Verifică dacă user-ul există deja
    let adminUser = await User.findOne({ 
      $or: [
        { name: "admindepozit" },
        { name: { $regex: /admin.*depozit/i } }
      ]
    });
    ...
    if (adminUser) {
      ...
      await adminUser.save();
      console.log(`   ✅ User 'admindepozit' creat cu succes (ID: ${adminUser._id})`);
