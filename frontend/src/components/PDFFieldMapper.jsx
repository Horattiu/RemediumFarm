 ...
 */
const PDFFieldMapper = ({ onSave, onCancel }) => {
  ...
  // Desenează marker-ele pentru câmpurile mapate
  const drawFieldMarkers = (ctx, canvasWidth, canvasHeight) => {
  ...
  const handleCanvasClick = (e) => {
    ...
    const fieldData = {
      ...
      fontSize: fieldNames.find(f => f.key === currentField)?.key.includes('checkbox') ? 10 : 11,
    };
  ...
  const adjustField = (fieldKey, axis, delta) => {
    setFields(prev => {
  ...
  const handleSave = async () => {
    ...
    const template = {
      pageHeight: viewport?.height || 841.89, // A4 height in points
      fields: fields,
      version: '2.0',
  ...
  const isFieldMapped = (key) => fields[key] !== undefined;
